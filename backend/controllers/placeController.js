...
const getPlaces = async (req, res) => {
  try {
    ...
    console.error("🔥 ASLI ERROR YAHAN HAI:", err); // 👈 Yeh nayi line add karo!
...
const addPlace = async (req, res) => {
  try {
    ...
    // 🕵️‍♂️ 3. THE AI BOUNCER CHECK (Calling Python Microservice)
    let generatedHashtags = [];
    try {
      console.log(`🚀 Checking with AI Security Bouncer for: ${localName}...`);
      
      const aiResponse = await fetch("http://localhost:8000/api/analyze-place", {
        method: "POST",
      ...
      // 🚨 AGAR AI NE REJECT KIYA:
      if (aiData.isApproved === false) {
        console.log(`❌ Place Rejected by AI: ${aiData.reason}`);
        
        // Kachra photo ko server (local storage) se uda do taaki space waste na ho
        if (req.file && fs.existsSync(req.file.path)) {
          fs.unlinkSync(req.file.path);
        }

        // Frontend ko gussa bhej do! (403 Forbidden)
      ...
      // ✅ Agar approve hua, toh AI ke diye hue hashtags save kar lo
      if (aiData.hashtags) {
        generatedHashtags = aiData.hashtags;
      }

    } catch (aiError) {
      console.error("❌ Python Microservice is down or error:", aiError.message);
      // Agar Python server band hai, toh app crash na ho isliye hum flow continue karne de rahe hain. 
      // Tum chaho toh yahan bhi error throw kar sakte ho production mein.
    }

    // ☁️ 4. Upload to Cloudinary (Ab sirf genuine jagahon ka upload hoga!)
    console.log("✅ AI Approved or Skipped! Uploading image to Cloudinary...");
    const result = await cloudinary.uploader.upload(req.file.path, {
      folder: "khojindia",
    });
    const mediaUrl = result.secure_url;

    // 5. Local file delete karo kyunki Cloudinary pe chali gayi
    ...
    const newPlace = new Place({
      ...
      hashtags: generatedHashtags, // 👈 AI se aaye tags yahan jud gaye!
    });
...
// ✅ 4. AI VIBE CHECKER (Naya Function)
const getPlaceVibe = async (req, res) => {
  try {
    ...
    const aiResponse = await fetch("http://localhost:8000/api/summarize-reviews", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
module.exports = { getPlaces, addPlace, searchPlaces, getPlaceById, addReview, getReviews, getPlaceVibe };